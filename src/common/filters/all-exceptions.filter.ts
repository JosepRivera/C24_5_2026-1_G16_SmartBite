...
export class AllExceptionsFilter implements ExceptionFilter {
	...
	private resolveError(exception: unknown): { statusCode: number; code: string; message: string } {
		if (exception instanceof HttpException) {
			...
			return { statusCode: status, code: exception.constructor.name.replace("Exception", "").toUpperCase(), message };
		}
	...
	private mapPrismaKnownError(
		...
		switch (exception.code) {
			...
			case "P2003":
				return { statusCode: 409, code: "CONFLICT", message: "Operation references a record that does not exist" };
			case "P2000":
			...
			case "P2014":
				return { statusCode: 400, code: "BAD_REQUEST", message: "Operation violates a required relation" };
			default:
				return { statusCode: 500, code: "INTERNAL_SERVER_ERROR", message: "Internal server error" };
		}
	}
